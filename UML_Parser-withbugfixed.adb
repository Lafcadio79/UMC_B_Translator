...
package body UML_Parser is
   ...
   ----------------------------------------------------
   HT: Character := Character'Val(9);  -- Horizontal Tab
   CR: Character := Character'Val(13); -- Horizontal Tab
   LF: Character := Character'Val(10); -- Horizontal Tab
   ...
   --  data to be saved/restored in case of multiple file parsing
   ...
   begin
      if Current_Token = 0 then
         Give_Error ("138 File Empty ??? ");
         raise Parsing_Error;
      elsif Current_Token = Tokens.all'Length then
         Give_Error ("141 Unexpected End of File");
         raise Parsing_Error;
         ...
         Give_Error ("148 Failure while looking for token " &  Item);
         raise Parsing_Error;
      end if;
   end Skip_Token;
   ...
   begin
      ...
      while Line_Length =0 loop
         ...
         if  Line_Length > 0 then
            ...
            --
            for I in 1..Line_Length loop
               if Character'Pos(Input_Line(I)) > 126 or
                 ( Character'Pos(Input_Line(I)) < 32 and
                    Character'Pos(Input_Line(I)) /= 9 and  --  ht
                      Character'Pos(Input_Line(I)) /= 10 and -- lf
                    Character'Pos(Input_Line(I)) /= 13     -- cr
                 ) then
                  Give_error("287 Error: Non printable character at line" &
                               Integer'Image(Line_Number) & ", column " &
                               Integer'Image(I) & "." );
                  raise Parsing_Error;
               end if;
               if I < Line_Length and
                 Input_Line(I) ='&' and Input_Line(I+1) ='#' then
                  -- this is an HTML encoding of a non printable character
                  Give_error("295 Error: Non printable character at  line" &
                               Integer'Image(Line_Number) & ", column " &
                               Integer'Image(I) & "." );
                  raise Parsing_Error;
               end if;
   ...
   begin
      ...
      for I in Current_Position .. Line_Length loop
         ...
         when Str =>
            ...
            if Item_Size > 1 and then Input_line(Current_Position) = '"' then
               ...
               Give_Error ("426 Unterminated String Literal");
               raise Parsing_Error;
            end if;
            -- continue the cycle
            --
      ...
      if This_Token.Item.all ="False" then This_Token.Item(1):= 'f'; end if;
      if This_Token.Item.all ="True" then This_Token.Item(1):= 't'; end if;
           ...
           This_Size  = Tmp_Size then
            if TheseEvents(I).Kind /= Undefined and then
              TheseEvents(I).Kind /= This_Event.Kind then
               Give_Error("897 Error: cannod declare a signal and an operation with the same name and args count");
   ...
   begin
      ...
      if not Lazy_Parsing then
         Give_Error("1017 Error: Found use of undeclared event " & This_Event.Name.all);
         raise Parsing_Error;
      else
         All_Classes(Current_Class).ChartEvents :=
         ...
         begin
            ...
            if Current_Item = "," or Current_Item = ";" then
               ...
               if  Current_Item /= "end" and then
                 ...
                 Next_Item /= "->" then
                  Give_Error("1176 Error: Expecting "","" or "";"" or Vars Section or State definitions.");
   ...
   -- x,y,z:=34;       -- DISALLOWED
   -- x:int, y:bool  -- SHOULD BE ALLLOWED USE of ',' instead of ';'
   -- x,y:int,       -- DISALLOWED  BECAUSE AMBIGUOUS
   function Parse_VarDecl return Vars_Table is
      This_Table: Vars_Table(1..100);
      Vars_Count:Natural :=0;
      This_Kind: String_Ref;
   ...
   begin
      ...
      This_Table(Vars_Count):= This_Var;
      ...
      --
      -- This_Var is the last declared variable of the table;
      --
      if Current_Item = ":" then
         -- parse the type [and initial value]
         Skip_Token;
         if Current_Item(1) in '0'..'9' and then
           Next_Item = ".." and then
           Next_Next_Item(1) in '0'..'9'  then
            This_Var.Kind :=  BoundedNat;
            This_Var.Initial := Zero.Simple;
            Skip_Token;  -- "0"
            Skip_Token;  -- ".."
            This_Var.NatBound :=  Integer'Value(Current_Item);
            Skip_Token;
         else
            This_Kind := Parse_Id;
            if This_kind.all = "obj" then
      ...
      if Current_Item = ":=" or else Current_Item = "=" then
         ...
         --
         -- CHECK  SIMPLE_EXPR DOES NOT CONTAIN INDEXING, HEAD, TAIL, LENGTH, UNDEFINEDNAMES
         --
         if Current_Item = "or" or else
           ...
           Current_Item(1) = '|' or else
           Current_Item(1) = '*' then
            Give_Error("1666 no operators allowed in this context.");
            raise Parsing_Error;
         end if;
         if Current_Item = "." then
            Give_Error("1670 function calls not allowed.");
            raise Parsing_Error;
         end if;
         --
      ...
      --
      for V in 1.. Vars_Count-1 loop
         This_Table(V).Kind := This_Var.Kind;
         This_Table(V).StaticSize := This_Var.StaticSize;
         This_Table(V).NatBound := This_Var.NatBound;
         This_Table(V).Initial := This_Var.Initial;
         This_Table(V).Typeinfo := This_Var.Typeinfo;
      end loop;
      --
      return This_Table(1..Vars_Count);
   end  Parse_VarDecl;
   ...
   begin
      ...
      --
      -- Vars   x y w w  --  TO BE DISALLOWED  !!!!!
      -- Vars   x,y,r :=234;
      -- Vars   x y,z,w:int :=3;  --  !!! only w is initialised!!!!
      -- Vars   x y;z;w
      ...
      loop   -- form each variable declaration
            ...
            for V in This_Vars_Table'Range loop 
               ...
               for I in All_Classes(Current_Class).ChartVars.all'Range loop
                  if All_Classes(Current_Class).ChartVars(I).Name.all =  This_Var.Name.all then
                     ...
                     --This_Var:= null;
                     --exit;
                     Give_Error("1752 Variable " & This_Var.Name.all & " already defined ");
                     raise Parsing_Error;
                  end if;
               end loop;
         ...
         --
         if  -- end of sequence of variables (maybe empty region)
           Current_Item="Vars" or
           Current_Item="vars" or
           Current_Item="Signals" or
           Current_Item="signals" or
           ...
           Current_Item="transitions"  then
            ...
            Give_Error("1813  "";"" expected as variable decl separator");
            raise Parsing_Error;
         else  -- Current_Item = ";"
            Skip_Token;
         end if;
   ...
   begin
      if Tokens(Current_Token).Kind /= Id then
         Give_Error("1854 State id expected");
         raise Parsing_Error;
      end if;
      This_Ident := Parse_Id;
      --
      if Parent_State.SubStates /= null and then
        ...
        This_Ident.all = "initial" then
         Give_Error ("1862 ""initial"" must be the first of a substates list.");
         raise Parsing_Error;
      end if;
      --
      ...
      if Current_Item = "(" and then Next_Next_Item /= "[" then  -- this is a composite substate
         ...
         if Parent_State.Kind /= Parallel then
            Give_Error ("1889 Error: Unexpected Region state inside composite sequential state");
            raise Parsing_Error;
         end if;
         Current_State.Priority := Parent_State.Priority + 1;
         Skip_Token ( "[" );
   ...
   begin
      if Tokens(Current_Token).Kind /= Id then
         Give_Error("1910 State id expected");
         raise Parsing_Error;
      end if;
      This_Ident := Parse_Id;
      --
      ...
      if Current_Item = "[" then  -- this is a composite substate
         if Parent_State.Parent=null or else
           Parent_State.Parent.Kind = Parallel then
            Give_Error ("1926 Error:  Concurrent regions must appear inside a composite sequentual state");
            raise Parsing_Error;
         end if;
         if Parent_State.Kind /= Parallel then
            Give_Error ("1930 Error: Unexpected Region state inside composite sequential state");
            raise Parsing_Error;
         end if;
         Skip_Token ( "[" );
         Parse_Substates_List(Current_State);
         ...
         -- the given name is a unique suffix
           ...
           FullName(FullName'Length-This_State_Name'Length) = '.' then
            if Result = null then
               ...
               Give_Error("1981 Error: Ambiguous State Name (" &
                            FullName.all & " or " &
         ...
         -- the given name is a unique suffix
           ...
           FullName(FullName'Length-This_State_Name'Length) = '.' then
            if Result = null then
               ...
               Give_Error("2046 Error: Ambiguous State Name (" &
                            FullName.all & " or " &
                            Result.FullName.all & ") - please use a full pathname.");
               raise Parsing_Error;
            end if;
         ...
         -- the given name is a unique suffix
           This_State_Name = These_States(I).FullName(
            ...
            if Result = null then
               ...
               Give_Error("2133 Error: Ambiguous State Name (" &
   ...
   begin
      ...
      if Current_Object > 2 and then
        All_Charts(Current_Object).ChartParent > 0 then
         Give_Error ("2270 Error: Object " & This_Object_Name.all &
                       " already defined!");
         raise Parsing_Error;
      end if;
      --
      if Current_Object in 1..2  and then
        All_Charts(Current_Object).Top_State_Num > 0 then
         Give_Error ("2277 Error: Object " & This_Object_Name.all &
                       " already defined!");
         raise Parsing_Error;
      end if;
      --
      ...
      if Current_Class = 0 then
         Give_Error ("2296 Error: Chart " & This_Class_Name.all & " not yet defined!");
         raise Parsing_Error;
      end if;
      ...
      All_Charts(Current_Object).IsRandom := All_Classes(Current_Class).IsRandom;
      ...
      for I in 1..Vars_Count loop
         ...
         This_Var.StaticSize := All_Classes(Current_Class).ChartVars(I).StaticSize;
         This_Var.NatBound := All_Classes(Current_Class).ChartVars(I).NatBound;
      ...
      if Current_Item  = "(" then
         ...
         loop
            ...
            begin
               if This_Var = 0 then
                  Give_Error ("2385 Error: Undefined attribute name """ &This_Item & """");
                  raise Parsing_Error;
               end if;
               if Current_Item  = "=>" or
                 Current_Item  = "="  or
               ...
               if Current_Item = "." then
                  Give_Error("2400 Attribute selection not allowed.");
                  raise Parsing_Error;
               end if;
            ...
            if Current_Item /= "," then
               Give_Error("2424 ERROR: Expecting a "","" or a "")"" to continue or end the parameters list.");
               raise Parsing_Error;
            end if;
            Skip_Token (",");
         end loop;
   ...
   begin
      ...
      begin
         ...
         Close(F);
         if Current_Item = "" then
            Give_Error ("2513 The UML file is EMPTY ...");
            raise Parsing_Error;
         end if;
      exception
         when others =>
            Give_Error ("2518 The UML file is not readable ...");
      ...
      ParseIncluded(F);
      --
      if New_Observations.all'Length=0 then
         All_Observations := new Observations_Table'(Default_Rules & Error_Rules);
      else
         All_Observations := new Observations_Table'(Error_Rules & New_Observations.all);
      end if;
      --
      ...
      if Active_Charts.all'Length < 1 then
         ...
         Put_line(Current_Error,"2535  Error: No object declaration found !!!! ");
         raise Parsing_Error;
      end if;
      --
         ...
         begin
            ...
            if Current_Item = "Chart" then
              ...
              (Current_Item = "Abstractions" or else
               ...
               Skip_Token ("{");
               New_Observations :=
                 new Observations_Table'(Parse_Observations(New_Observations.all));
               Skip_Token("}");
               ...
               if Included_File_Name /= null then
                  Give_Error("2651 #include directive can only appear in main system file");
                  Parsing_Errors := Parsing_Errors+1;
                  raise Parsing_Error;
               end if;
               ...
               begin
                  ...
                  Close(Fincluded);
                  if Current_Item = "" then
                     Give_Error ("2688 The UML file is EMPTY ...");
                     raise Parsing_Error;
                  end if;
                  -------
               ...
               exception
                  when others =>
                     Give_Error ("2696 The UML file is not readable ...");
               ...
               Give_Error( "2716 Class  or Object  declaration expected");
               Parsing_Failed := True;
               if Parsing_Errors > 5 then
                  raise Parsing_Error;
               else
   ...
   begin
      if Ids'Length =2 then
         ...
         if Result = 0 then
            Give_Error ("2755 Object name expected ...  (obj.attr)" );
            raise Parsing_Error;
         end if;
         ...
         Give_Error ("2765 Attribute name expected ...  (obj.attr)" );
         raise Parsing_Error;
         --
         ...
         if Is_Number(Ids(1).all) or else
           ...
           Ids(1).all = "maxqueuesize"  then
            ...
            Give_Error ("2785 Literal or object name expected .. " );
            raise Parsing_Error;
         end if;
      end if;
      --
      ...
      for J in This'Range loop
         ...
         if  This(J).LTerm/= null and then
           This(J).LTerm.all = Varname then
            Give_Error("2799 Illegal multiple use  of variable in observation rule");
            raise Parsing_Error;
         end if;
         if This(J).LPartner /= null and then
           This(J).LPartner.all = Varname then
            Give_Error("2804 Illegal multiple use of variable in observation rule");
            raise Parsing_Error;
         end if;
         if This(J).LOp /= null and then
           This(J).LOp.all = Varname then
            Give_Error("2809 Illegal multiple use of variable in observation rule");
            raise Parsing_Error;
         end if;
         if This(J).LArgs /= null then
            for A in This(J).LArgs.all'Range loop
               if This(J).LArgs(A).all = Varname then
                  Give_Error("2815 Illegal multiple use of variable in observation rule");
                  raise Parsing_Error;
               end if;
            end loop;
         end if;
         ...
         if This(J).IdsR /= null then
            for I in This(J).IdsR.all'Range loop
               if This(J).IdsR(I).all = Varname then
                  Give_Error("2824 Illegal multiple use  use of variable in observation rule");
                  raise Parsing_Error;
               end if;
            end loop;
         end if;
   ...
   begin
      ...
      --
      Give_Error ("2866 Abstraction Rule contains undefined variables ...");
      raise Parsing_Error;
      --
   end Check_Used;
   ...
   begin
      ...
      --    foo($1,$2)  and $1 = $2  -> bb
      --
      --  Left_Rule_Element = [ LTerm, LPartner, Lop, LArgs, / IdsL , LeftOp, IdsR ]
      --                    =    obj:   tgt.     sig  v1,v2     1,2     EQ    $1,$2
      --
      --  GOAL:  allow multiple $ defining 'foo($1,$2)'  'bar($3,$2)'
      --  GOAL   $ defining realtions  $1[1][2] = $3  and  $1[$3] = $4 and ...
      --
      ...
      if Op.Kind= ActionKind then
         if OP.Left(1).LTerm /= null and then
           ...
           OP.Left(1).LPartner.all = OP.Left(1).LTerm.all then
            Give_Error("2897 Illegal multiple use  use of variable in observation rule");
            raise Parsing_Error;
         end if;
         if OP.Left(1).LTerm /= null and then
           ...
           OP.Left(1).LOp.all = OP.Left(1).LTerm.all then
            Give_Error("2904 Illegal multiple use  use of variable in observation rule");
         end if;
         if OP.Left(1).LPartner /= null and then
           OP.Left(1).LOp /= null and then
           OP.Left(1).LOp(1) = '$' and then
           OP.Left(1).LOp.all = OP.Left(1).LPartner.all then
            Give_Error("2910 Illegal multiple use  use of variable in observation rule");
            raise Parsing_Error;
         end if;
         --
         --
      ...
      if Op.Kind /= ActionKind then  -- i.e. StateKind
            ...
            if OP.Left(I).Largs /= null then
               for J in OP.Left(I).Largs.all'Range loop
                  if OP.Left(I).Largs(J)(1) = '$' then
                     Give_Error("2935 Error: no $ variables allowed in this context");
                     raise Parsing_Error;
                  end if;
               end loop;
            end if;
            ...
            if OP.Left(I).IdsL /= null then
               ...
               for J in OP.Left(I).IdsL.all'Range loop
                  if OP.Left(I).IdsL(J)(1) = '$' then
                     ...
                     if not OK then
                        Give_Error("2959 Error: no $ variables allowed in this context");
                        raise Parsing_Error;
                     end if;
                  end if;
               end loop;
                 ...
                 OP.Left(I).IdsR(1)(1) = '$' then
                  ...
                  for J in OP.Left(I).IdsR.all'Range loop
                     if OP.Left(I).IdsR(J)(1) = '$' then
                        ...
                        if not OK then
                           Give_Error("2993 Error: no $ variables allowed in this context");
                           raise Parsing_Error;
                        end if;
                     end if;
         ...
         for RL in OP.Rlabels.all'Range loop
            if OP.Rlabels(RL)(1)= '$' then
               ...
               if not OK then
                  Give_Error("3014 Error: $ variable not defined  ");
                  raise Parsing_Error;
               end if;
            end if;
         end loop;
   ...
   ------------------------------------------------------------------------
   -- Parse_Observations =
   --    Parse_Left_Side (Kind, Empty_Left_Side)  ->  ..... ;
   --    Parse_observations;
   --
   -- Parse_Left_Side (Kind, Previous) =
   --      " ( This )   and  ... "
   --     Parse_Left_Side (Kind, Previous & This);
   --
   --
   ...
   --    foo($1,$2)  and $1 = $2  -> bb
   --
   --  Rule_Left_Elem = [ LTerm, LPartner, Lop, LArgs, / IdsL , LeftOp, IdsR ]
   --                    =    obj:   tgt.     sig  v1,v2     1,2     EQ    $1,$2
   --
   --  GOAL:  allow multiple $ defining 'foo($1,$2)'  'bar($3,$2)'
   -------------------------------------------------------------------
   -- N.B  Rule_Left  is a vector of Rule_Left_Elem;
   --
   -------------------------------------------------------------------
   function Parse_Left_Side (Kind: AbstractionKind;
      ...
      This: Rule_Left_Elem;
      OK: Boolean;
      chartindex: Natural;  -- used for the obj index  in   State obj.v = ...
      Tmp: Natural;
   begin
      ...
      if Kind = ActionKind then
         ...
         --  riscritta ???????????
         --   I.e.   Action    obj1:tgt1.foo and obj1:tgt2.bar  ->  foobar
         --    both tgt1  and tgt2  should go into the unique  rule_elem.LPartner field.
         --
         if  Previous'Length =0  then
            ...
            if   Next_Item =":" then
               ...
               if  Current_Item = "$*" then
                  Give_Error("3139  ""$*"" not allowed in this context.");
                  raise Parsing_error;
               end if;
               Skip_Token;  -- obj:
               Skip_Token (":");
            ...
            if Next_Item ="." then
               ...
               if  Current_Item = "$*" then
                  Give_Error("3150  ""$*"" not allowed in this context.");
                  raise Parsing_error;
               end if;
               Skip_Token;   -- target.
               Skip_Token (".");
            ...
            if Next_Item = "(" or else
              Next_Item = "->" or else
              Next_Item =  "and" then
               This.LOp := Tokens(Current_Token).Item;
               if  Current_Item = "$*" then
                  Give_Error("3163  ""$*"" not allowed in this context.");
                  raise Parsing_error;
               end if;
               --  We want to allow   Action $1($2,$*) -> $1($2)
               --  but we must later check that $1 is actually associated to action event name!
               --        if  Current_Item(1) not in 'a'..'z' and then Current_Item(1) not in 'Z'..'Z'  then
               --          Give_Error("Error: signal identifier expected");
               --          raise Parsing_error;
               --        end if;
               Skip_Token;   -- foo (
               --
               ...
               if This.LOp.all =  "assign"  and then Current_Item /= "(" then
                  Give_Error("3176  Incorrect structure of assign rule");
                  raise Parsing_error;
               end if;
               --
               ...
               if Current_Item ="(" then
                  ...
                  if (This.LOp.all = "discarded" or
                      ...
                      Current_Item = "$*" then
                     Give_Error("3187  ""$*"" not allowed in this context.");
                     raise Parsing_error;
                  end if;
                  if ( This.LOp.all =  "assign") and then
                    Current_Item(1) = '$' then
                     Give_Error("3192  ""$vars"" not allowed in this context.");
                     raise Parsing_error;
                  end if;
                  This.LArgs := new String_table'(Empty_String_Table);
                  while Current_Item /= ")" loop
                     if Current_Item(1) ='$' or else
                       Current_Item(1) in '0'..'9' or else
                       Current_Item(1) in 'a'..'z' or else
                       Current_Item(1) in 'A'..'Z' or else
                       Current_Item(1) ='*' then
                        This.LArgs := new String_Table'(This.LArgs.all & Tokens(Current_Token).Item);
                     else
                        if Current_Item(1) = '[' then
                           Give_Error("3205 Error: vector literals not allowed in this context");
                           raise Parsing_Error;
                        else
                           Give_Error("3208 Error: expecting '*' or '$var' or varname or simple literal");
                           raise Parsing_Error;
                        end if;
                     end if;
                     Skip_Token;
                     ...
                     --    Skip_Token;
                     if Current_Item = "," and then Next_Item /= ")" then
                        Skip_Token;
                     elsif Current_Item /= ")" then
                        Give_Error("3218 Error: ',' or ')' expected");
                        raise Parsing_Error;
                     end if;
                  end loop;
                  Skip_Token (")");
                  if ( This.LOp.all =  "assign") then
                     if This.LArgs.all'Length <2 then
                        Give_Error("3225  Incorrect structure of assign rule");
                        raise Parsing_error;
                     else
            ...
            end if;  -- parse eventname and params
            --
            if This.LOp= null and This.LPartner = null and This.LTerm =null then
               Give_Error("3235 Error: Incorrect structure of abstraction rule: expecting '[src:][tgt.]event(args) [and $var relop expr]* ->  ...' ");
               raise Parsing_error;
            end if;
            --
            if Current_Item = "->" then
               ...
               Give_Error("3246 Error: expectiong '->' or 'and' ");
               raise Parsing_error;
            end if;
            --
         end if; -- Previous.Length=0
         --
         -------------------------------------------------------------------
         ...
         -------------------------------------------------------------------
         --  NOW: $var1 = 123  and  $var2 = $var3  (all vars defined)
         --  GOAL   $ defining realtions  $1[1][2] = $3  and  $1[$3] = $4 and ...
         -------------------------------------------------------------------
         --
         -- PARSE LEFT SIDE OF RELATION:  $1,  $1[1].length, $1[2].tail.[3].head
         --
         -------------------------------------------------------------------
         if Current_item(1) ='$' and then  not Has_Been_Defined(Current_item, Previous) then
            Give_Error("3265  """& Current_item &""" not defined");
            raise Parsing_error;
         elsif Current_item(1) /='$' and then Current_item(1) not in '0'..'9' and then
           Current_item(1) not in 'a'..'z' and then Current_item(1) not in 'a'..'z' then
            Give_Error("3269 Error:  expecting '$var' or 'basic literal'");
            raise Parsing_error;
         elsif (Current_item(1) in '0'..'9' or else
                Current_item(1) in 'a'..'z' or else Current_item(1) in 'a'..'z') and then
           (Next_item ="[" or else Next_item = "." or else
            Next_item = "+"  or else Next_item = "-" ) then
            Give_Error("3275 Error:  only basic literals are allowed in this context ('literal relop expr')");
            raise Parsing_error;
         end if;
         This.IdsL := new String_Table'(1..1 => Tokens(Current_Token).Item);
         Skip_Token;  -- $var
         --
         -- parse optional indexings:  $1[1][2] =   $1[$3] =
         loop
            if Current_Item = "[" then
               Skip_Token; --'['
               if Current_Item(1) = '$' then
                  -- check $var defined
                  if not Has_Been_Defined(Current_Item, Previous) then
                     Give_Error("3288  """& Current_Item & """ not defined");
                     raise Parsing_error;
                  else
                     This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
                     Skip_Token;
                  end if;
               elsif Current_Item(1) in '0'..'9' then
                  --ok intger literal
                  This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
                  Skip_Token;
               else
                  Give_Error("3299 ERROR: expecting '$var' or numeric literal");
                  raise Parsing_error;
               end if;
               Skip_Token("]");
               --
               ...
               Skip_Token;
               if Current_Item="head" or Current_Item="tail" or Current_Item="length" then
                  This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
                  Skip_Token;
               else
                  Give_Error("3310 Error:  .head .tail .length .min .max expected");
                  raise Parsing_Error;
               end if;
            else
               exit;
         ...
         if Current_Item = "="  or else
           ...
           Current_Item = "/=" then
            ...
            This.LeftOp := GE;
            Skip_Token;
         elsif Current_Item = "in" then
            This.LeftOp := ISIN;
            Skip_Token;
         else
            Give_Error("3342 Error: Unexpected binary operator " & Current_Item );
            raise Parsing_Error;
         end if;
         --
         --  ...=   $1[1][$2]  (with $1, $2 old)
         --  ...=   obj  null, [] , 123, -1, true false
         --  ...=   $var  (with $var new!!)
         --
         -------------------------------------------------------------------
         --
         -- PARSE RIGHT SIDE OF RELATION:  $1,  $1[1], $1[2][3], undefined
         --
         -------------------------------------------------------------------
         --
         if Current_Item(1)='$' and then Next_Item(1)='['  then
            --
            --  ...=   $1[1][$2]  (with $1, $2 old)
            --
            if Has_Been_Defined(Current_Item, Previous) then
               -- parse expression:  $1[...
               This.IdsR := new String_Table'(1..1 => Tokens(Current_Token).Item);
               Skip_Token;
               -- parse optional indexings:  $1[1][2]
               loop
                  if Current_Item = "[" then
                     Skip_Token; --'['
                     if Current_Item(1) = '$' then
                        -- check $var defined
                        if not Has_Been_Defined(Current_Item, Previous) then
                           Give_Error("  """& Current_Item & """ not defined");
                           raise Parsing_error;
                        else
                        ...
                        Give_Error("3383 ERROR: expecting '$var' or numeric literal");
                        raise Parsing_error;
                     end if;
                     Skip_Token("]");
                     --
                     ...
                     if Current_Item="head" or Current_Item="tail" or Current_Item="length" then
                        ...
                        Give_Error("3394 Error:  .head .tail .length .min .max expected");
                        raise Parsing_Error;
                     end if;
                  else
                     exit;
            ...
            --  ...=   -123
         elsif Current_Item = "-" and then Next_Item(1) in '0'..'9' then
            Skip_Token;
            This.IdsR := new String_Table'
              (1..1 => new String'("-" & Tokens(Current_Token).Item.all));
            Skip_Token;
            --
         elsif Current_Item = "null" or
           Current_Item = "true" or
           Current_Item = "false" or
           Current_Item = "undefined" or
           Current_Item(1) in '0'..'9'  then
            This.IdsR := new String_Table'(1..1 => Tokens(Current_Token).Item);
            Skip_Token;
            --
         elsif Current_Item = "[" and then Next_Item="]" then
            This.IdsR := new String_Table'(1..1 => Tokens(Current_Token).Item);
            Skip_Token;
            This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
            Skip_Token;
            ...
            --  now $var  can be either new or old.
            This.IdsR := new String_Table'(1..1 => Tokens(Current_Token).Item);
            Skip_Token;
            --
         elsif Current_Item(1) in 'a'..'z' or else Current_Item(1) in 'A'..'Z' then
            -- check the item is a ChartName
            This.IdsR := new String_Table'(1..1 => Tokens(Current_Token).Item);
            Tmp :=0;
            for K in All_Charts.all'Range loop
               if This.IdsR(1).all = All_Charts(K).Name.all then
                  Tmp :=K;
                  exit;
               end if;
            end loop;
            if Tmp=0 then
               for K in All_Classes.all'Range loop
                  for EE in All_Classes(K).ChartEvents.all'Range loop
                     if This.IdsR(1).all = All_Classes(K).ChartEvents(EE).Name.all then
                        Tmp :=K;
                        exit;
                     end if;
                  end loop;
               ...
               if Tmp=0 then
                  Give_Error("3452 ERROR:  object or token name (or event name) expected ...");
                  raise Parsing_Error;
               end if;
            end if;
            Skip_Token;
         ...
         if Current_Item = "->" then
            ...
            Give_Error("3468 ""and""  or ""->"" expected (action rules can at most be conjunctions)");
            raise Parsing_Error;
         end if;
         --
      end if; -- actionKind
      ...
      ----------
      --
      --  LEFT_RULE_HEAD = final || isState() || (Obj.svar || svar || $var) LEFT_RULE_TAIL
      --  LEFT_RULE_TAIL = ([..] ||  .length  || .head) LEFT_RULE_TAIL
      --
      if Current_Item = "final" then
         --
         --  LEFT_RULE_HEAD = final || ...
         --
         Skip_Token;
      ...
      if Current_Item = "inState" or
        Current_Item = "InState" then
         --
         --  LEFT_RULE_HEAD = ... inState(obj,s1)/inState(s1)/inState(obj,s1) ...
         --
         Skip_Token;
         Skip_Token("(");
         This.LOp := Tokens(Current_Token).Item;   -- obj (or s1 if obj is implicit)
         --  check obj explicit  or implicit or error
         OK := False;
         for I in UML_Types.Active_Charts.all'Range loop
            if UML_Types.All_Charts(UML_Types.Active_Charts(I)).Name.all = This.LOp.all then
               OK := True;
               Skip_Token;  --obj
               if Current_Item = "." or else
                 Current_Item = "," then
         ...
         if not OK then
            ...
            if UML_Types.Active_Charts.all'Length=1 then
               OK := True;
               This.LOp := UML_Types.All_Charts(UML_Types.Active_Charts(1)).Name;
            else
               Give_Error("3539 Error in Abstraction expression: missing object prefix");
               raise Parsing_Error;
            end if;
         end if;
         --
         while Current_Item /=")" loop           --  top.S1.s2
            ...
            if Current_Item ="." then
               ...
               Give_error("3554  '.' or ')' expected)");
               raise Parsing_Error;
            end if;
         end loop;
         if This.LArgs = null then
            Give_Error("3559 Expecting 'inState(obj,statename)'");
            raise Parsing_Error;
         end if;
         Skip_Token (")");
         --
      ...
      --  $1 <= $2
      --
      --  LEFT_RULE_HEAD =    ... (Obj.svar || svar || $var) LEFT_RULE_TAIL
      --
      if Current_Item="["  then   -- "["   emptyvector literal
         --      if Next_Item = "]" then
         --         This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
         --         Skip_Token;
         --         This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
         --         Skip_Token;
         --      else
         Give_Error("3594 Error:  vector literals not allowed in this context");
         raise Parsing_Error;
         --      end if;
         --
      elsif Current_Item(1) = '$' then
         -- $var  as beginning  of left_side
         ...
         if not OK then
            Give_Error("3611 Found use of undefined $variable " & Current_Item);
            raise Parsing_Error;
         end if;
         This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
         Skip_Token;
         --
      elsif Current_Item= "true" or Current_Item= "false" or Current_Item= "True" or Current_Item= "False" then
         -- this is just a boolean literal
         This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
         Skip_Token;
         --
      elsif Current_Item(1) in '0'..'9' or else
        (Current_Item'Last >1 and then
         Current_Item(1)='-' and then
         Current_Item(2) in '1'..'9') then
         -- this is just a numeric literal
         This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
         Skip_Token;
         --
      else
         ...
         begin
            if Current_Item = "null" then
               ...
               Skip_Token;
               --
            elsif Current_Item = "maxqueuesize" then
               This.IdsL := new String_Table'(1 => Tokens(Current_Token).Item);
               Skip_Token;
               chartindex :=0;
            else
               for Z in Active_Charts.all'Range loop
                  if All_Charts(Active_Charts(Z)).Name.all = Current_Item then
                     isobjectname := True;
                     chartindex := Active_Charts(Z);
                     This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
                     Skip_Token;  -- object name
                     if Current_Item = "." then
                        Skip_Token(".");
                     end if;  -- Current_Item = "."
                     exit;
               ...
               if not isobjectname then
                  if Active_Charts.all'Length >1 then
                     -- give_error
                     Give_Error("3665 Error: Missing Object Name in class attribute selection.");
                     raise Parsing_Error;
                  elsif Active_Charts.all'Length =0 then
                     -- give_error
                     Give_Error("3669 Error: No active object declaration found !!!! ");
                     raise Parsing_Error;
                  else
                     ...
                     chartindex := Active_Charts(1);
                  end if;
               end if;
            end if;
         end;
         --
         --  check  not obj[] null.  null[]
         if chartindex >0 and then
           (Current_Item(1) in 'a'..'z' or Current_Item(1) in 'A'..'Z') then
            --
            --  CHECK THAT  The var name actually denotes an existing class attribute!!
            --
            if Current_Item /= "queuesize" then
               OK := False;
               for K in All_Charts(chartindex).ChartVars.all'Range loop
                  if Current_Item = All_Charts(chartindex).ChartVars(K).Name.all  then
                     OK := True;
                     exit;
                  end if;
               end loop;
               if not OK then
                  Give_Error("3697 Error: variable " & Current_Item & " not declared.");
                  raise Parsing_Error;
               end if;
            end if;
            This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
            Skip_Token; -- the  var name          --
         end if; -- not relational operator
         --
      end if;  -- $var or obj or literal
      --
      --  LEFT_RULE_TAIL = ([..] || .length || .head) LEFT_RULE_TAIL
      --
      loop
         if Current_Item = "[" then
            ...
            if Current_Item(1) = '$'  then
               ...
               if not OK then
                  Give_Error("3725 Found use of undefined variable " & Current_Item);
                  raise Parsing_Error;
               end if;
               This.IdsL := new String_Table'(This.IdsL.all & Tokens(Current_Token).Item);
               Skip_Token;
            ...
            if Current_Item="head" or Current_Item="tail" or Current_Item="length" 
              or Current_Item="min" or Current_Item="max" then
               ...
               Give_Error("3743 Error:  '.head' '.tail' '.length' '.min' '.max' expected");
               raise Parsing_Error;
            end if;
         else
            exit;
      ...
      if Current_Item = "<=" then
         ...
         Give_Error ("3783 Relational operator expected");
         raise Parsing_Error;
      end if;
      --
      ----------  PARSE RIGHT SIDE OF RELATION
      --  "[]"  at rigth side
      if Current_Item="["  then
         if Next_Item = "]" then
            This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
            ...
            Give_Error("3792 Error:  vector literals not allowed in this context");
            raise Parsing_Error;
         end if;
         --
         --  "undefined" at right side
      elsif Current_Item = "undefined"  then
         This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
         Skip_Token;
         --
      elsif Current_Item(1) = '$' then
         ...
         if not OK then
            if Next_Item(1)='[' or else Next_Item(1)='.' then
               Give_Error("3813 Found use of undefined $variable " & Current_Item);
               raise Parsing_Error;
            else
               -- this is a new dollarvar definition
               This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
               Skip_Token;
            end if;
         else
            -- this is the HeadRight
            This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
            Skip_Token;
         end if;
         --
        ...
        (Current_Item'Last >1 and then
         ...
         -- check Current_Item is an object name (active, Token , ...)
         --   obj = ...
         --   obj.var = ...
         --   var =  ...  (1 object, implicit)
         declare
            isactiveobjectname:Boolean := False;
         begin
            if Current_Item="null" then
               isactiveobjectname := True;
               chartindex :=0;
               This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
               Skip_Token;
            else
            ...
            if not isactiveobjectname and Current_Item = "." then
               if Active_Charts.all'Length >1 then
                  -- give_error
                  Give_Error("3869 Error: Missing Object Name in class attribute selection.");
                  raise Parsing_Error;
               elsif Active_Charts.all'Length =0 then
                  -- give_error
                  Give_Error("3873 Error: No active object declaration found !!!! ");
                  raise Parsing_Error;
               else
                  --  MISSING THE CHECK THAT  The var name actually denotes an existing class attribute!!
                  --
                  -- add implicit object name
                  --             Skip_Token(".");
                  This.IdsR := new String_Table'(This.IdsR.all & All_Charts(Active_Charts(1)).Name);
                  chartindex := Active_Charts(1);
               end if;
            end if;
         end;
         ...
         if Current_Item  /= "->" and then
           ...
           (Current_Item(1) in 'a'..'z' or 
            ...
            if Current_Item ="." then --new
               ...
               if not OK then
                  Give_Error("3908 Error: variable " & Current_Item & " not declared.");
                  raise Parsing_Error;
               end if;
            end if;
            This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
      ...
      --
      --  RIGHT_RULE_TAIL = ([..] || .length || .head) RIGHT_RULE_TAIL
      --
      loop
         if Current_Item = "[" then
            Skip_Token;
            if Current_Item(1) = '$'  then
               ...
               if not OK then
                  Give_Error("3936 Found use of undefined variable " & Current_Item);
                  raise Parsing_Error;
               end if;
               This.IdsR := new String_Table'(This.IdsR.all & Tokens(Current_Token).Item);
               Skip_Token;
            ...
            if Current_Item="head" or Current_Item="tail" or Current_Item="length" 
              or Current_Item="min" or Current_Item="max" then  -- ADDED APRIL 1 2019
               ...
               Give_Error("3954 Error: '.head' '.tail' '.length' '.max' '.min' expected");
               raise Parsing_Error;
            end if;
         else
            exit;
   ...
   begin
      ...
      if not Found then
         Give_Error("4011 Undefined variable " & D);
         raise Parsing_Error;
      end if;
   end Check_Dollar;
   ...
   begin
      ...
      --
      --  handle TRANSITION LABELS
      --
      if Current_Item ="TLABELS" then
         Observe_Transition_Labels := true;
         Skip_Token;
      end if;
      --
      ...
      if Current_Item ="State" or
        ...
        Current_Item ="action" then
         ...
         Give_Error("4060 State or Action keyword expected");
         raise Parsing_Error;
      end if;
      if Current_Item = ":" then
         Skip_Token(":");
      ...
      --
      --
      --  removed because              Action $1:* -> $1   should be allowed
      --   at least until  the worse:  State O2.v= $1 -> $1  is allowed!!!
      ...
      if Current_Item = "(" then
         ...
         Nesting :=0;
         while Current_Item /= ")" loop
            -- WE must disallow    $1[1], [1,2,3][2], $1.tail
            -- WE must allow  'name)' 'name,'   ' name]' '[name',
            -- I.E  '['    CANNOT FOLLOW a ']' or a name
            if Tokens(Current_Token).Item(1) ='$' then
               Check_Dollar(Op,Tokens(Current_Token).Item.all);
            end if;
            if Current_Item="." then
               --APRIL 1 2019
               Give_Error("4103 ERROR:   '.head' '.tail' '.length' '.max' '.min' not allowed in right side of rules");
               raise Parsing_Error;
            end if;
            if Current_Item ="[" then
               Nesting := Nesting+1;
               ...
               if Nesting > 0 then
                  ...
                  Give_Error("4112 ERROR:  unbalanced '[' ']' ???");
                  raise Parsing_Error;
               end if;
            end if;
            --        elsif Current_Item /= "$" or else
            ...
            --
            -- Items can be ']' '[' or 'names'  (',' alrady been eaten)
            --
            if Current_Item /= "[" and then Next_Item=  "[" then
               --  '['   CAN ONLY FOLLOW the initial '('  or ',' or '['
               --   ']['  ie error,  'id['  is error
               --
               Give_Error ("4126 ERROR:  indexing not allowed in right side of rules");
               raise Parsing_Error;
            end if;
            --
            --  can we check tht ',' is presente between two items?
            -- i.e. disallow   foo(aa bb [oo] [[]] [aa nn])
            -- We can force ',' or ')' or ']' to be present after a 'name'
            if Current_Item /= "[" and then
              Current_Item /= "]" and then
              Next_Item /= ")" and then
              Next_Item /= "]" and then
              Next_Item /= "," then
               Give_Error("4138 ERROR:  expecting ',' or ')' after '" & Current_Item & "'");
               raise Parsing_Error;
            end if;
            Skip_Token;   --  '['  ']'  'name'
            if Current_Item = "," then
               --  literal  .. number .. name .. token  ')' ']'
               Skip_Token;
            end if;
         end loop;
         --
         ...
         if Current_Item = "Defers" or else
           Current_Item = "defers" then
            ...
            if Current_State.Kind /= Simple then
               Give_Error("4423 Error: state " & This_State_Name.all & "  already defined.");
               raise Parsing_error;
            end if;
            --
   ...
   begin
      if Current_Item = "with" then
         Skip_Token;
         while To_Lower(Current_Item) /= "is" loop
            if To_Lower(Current_Item) = "niceparallelism" then
               isnice := True;
               Skip_Token;
               if Current_Item="," then Skip_Token; end if;
            elsif To_Lower(Current_Item) = "randomqueue" then
               israndom := True;
               Skip_Token;
               if Current_Item="," then Skip_Token; end if;
            else 
      ...
      All_Classes(Current_Class).IsRandom := israndom;
      --
      ...
      if Current_Item = ""  or else
        ...
        Current_Item = "class" then
         ...
         if Current_Item = ";" then
            ...
            Give_Error ("4653 Class Name mismatch in ""end"" clause");
            raise Parsing_Error;
         end if;
         return;
      end if;
      ...
      if Current_Item = "Signals" or
        ...
        Current_Item = "Vars" or
        Current_Item = "vars" then
         Give_Error("4676 Error: misplaced Vars section?");
         Parsing_Failed := True;
         if Parsing_Errors > 5 then
            raise Parsing_Error;
         else
      ...
      if Flags.Lazy_Parsing or Flags.Almost_Lazy_Parsing then
         ...
         if All_Classes(Current_Class).ChartStates.all'length >0 then
            ...
            Give_Error("4706 Error:  Active class without transitions ??? ");
            raise Parsing_Error;
         end if;
      end if;
      --
      if All_Classes(Current_Class).Top_State_Num = 0 then
         if Progress = 2 then
            Give_Error("4713 Error: ""Vars"" or ""State"" expected here");
         elsif Progress =3 then
            Give_Error("4715 Error: ""State"" or ""Transitions"" expected here");
         end if;
         Parsing_Failed := True;
         if Parsing_Errors > 5 then
            raise Parsing_Error;
      ...
      for I in All_Classes(Current_Class).ChartStates.all'Range loop
         if All_Classes(Current_Class).ChartStates(I).fullname.all'last > 8 and then
           ...
           (All_Classes(Current_Class).ChartStates(I).OutgoingTransitions = null or else
            ...
            Give_Error ("4745 Missing transition for default initial state");
            raise parsing_error;
         end if;
      end loop;
      --
   ...
   begin
      if Tokens(Current_Token).Kind = Id then
         ...
         Give_Error ("4991 Identifier Expected");
         raise Parsing_Error;
      end if;
   end Parse_Id;
   ...
   begin
      ...
      Give_Error ("5088 Parsing Error while looking for a int literal");
      raise Parsing_Error;
   end Parse_Int_Token;
   ...
   begin
      ...
      if This_Item = "["  then
         ...
         while Current_Item /= "]" loop
            ...
            if Current_Item = "," then
               ...
               Give_Error("5173 Error:   ',' or ']' expected");
               raise Parsing_Error;
            end if;
         end loop;
         ...
         --   CANNOT BE USED OUTSIDE  UMC EXECUTION ENV
         --  static valkua of vector litersl
         --  RIMOSSO PER UMC2B !!!!!!!!!!!!
         ...
         This_Simple.Image := new String'(This_Simple.Image.all & "]");
         if Current_Item = "[" then
            Give_Error("5185 Indexing not allowed in this context.");
            raise Parsing_Error;
         end if;
         if Current_Item = "." then
            Give_Error("5189 function calls not allowed in this context.");
            raise Parsing_Error;
         end if;
         return This_Simple;
      ...
      if Is_Number(This_Item) then
         ...
         if Current_Item = "[" then
            Give_Error("5203 Indexing not allowed in this context.");
            raise Parsing_Error;
         end if;
         if Current_Item = "." then
            Give_Error("5207 function calls not allowed in this context.");
            raise Parsing_Error;
         end if;
         return This_Simple;
      end if;
      ...
      if This_Item= "-" and then
        Is_Number(Current_Item) then
         ...
         Skip_Token;
         if Current_Item = "[" then
            Give_Error("5220 Indexing not allowed in this context.");
            raise Parsing_Error;
         end if;
         if Current_Item = "." then
            Give_Error("5224 function calls not allowed in this context.");
            raise Parsing_Error;
         end if;
         return This_Simple;
      ...
      --
      -- some kind of var or OBJECTNAME!!
      --
         ...
         while Current_Item = "." or Current_Item = "[" loop
            if Current_Item = "." and then
              ( Tokens(Current_Token+1).Item.all = "length" or
               ...
               return This_Simple;
               --
               --  if followed by "max" ... evaluate it
               --
            elsif Current_Item = "." and then
              ( Tokens(Current_Token+1).Item.all = "max" or
                   Tokens(Current_Token+1).Item.all = "Max")  then
               Skip_Token;   -- "."
               --  new 30-06
               Tmp := This_Simple;
               This_Simple := new SimpleIntExpr;
               This_Simple.Head_Tail_Data := Tmp;
               This_Simple.Special_Token := new String'("max");
               This_Simple.Image := new String '(This_Simple.Head_Tail_Data.Image.all & ".Max");
               This_Simple.Kind := Number;
               Skip_Token; -- "max"
               --
               --  if followed by "min" ... evaluate it
               --
            elsif Current_Item = "." and then
              ( Tokens(Current_Token+1).Item.all = "min" or
                   Tokens(Current_Token+1).Item.all = "Min")  then
               Skip_Token;   -- "."
               --  new 30-06
               Tmp := This_Simple;
               This_Simple := new SimpleIntExpr;
               This_Simple.Head_Tail_Data := Tmp;
               This_Simple.Special_Token := new String'("Min");
               This_Simple.Image := new String '(This_Simple.Head_Tail_Data.Image.all & ".Min");
               This_Simple.Kind := Number;
               Skip_Token; -- "min"
               --
               --  if followed by "toset" ... evaluate it
               --
            elsif Current_Item = "." and then
              ( Tokens(Current_Token+1).Item.all = "toset" or
                   Tokens(Current_Token+1).Item.all = "Toset")  then
               Skip_Token;   -- "."
               --  new 30-06
               Tmp := This_Simple;
               This_Simple := new SimpleIntExpr;
               This_Simple.Head_Tail_Data := Tmp;
               This_Simple.Special_Token := new String'("Toset");
               This_Simple.Image := new String '(This_Simple.Head_Tail_Data.Image.all & ".Toset");
               --
              ...
              ( Tokens(Current_Token+1).Item.all = "tail" or
               ...
               if This_Simple.Special_Token /= null and then
                 This_Simple.Special_Token.all = "Tail" then
                  Give_Error("5467 Indexing not allowed after tail.");
                  raise Parsing_Error;
                  --
               else
                  Tmp := This_Simple;
         ...
         while nesting /= 0 loop
            if Tokens(Index).Item.all = "["  then
              ...
              Tokens(Index).Item.all = "{" then
               Give_Error ("6013 Errror: Found token " & Tokens(Index).Item.all &
                             " while looking for a ""]""" );
               raise Parsing_Error;
            else
               Index := Index+1;
   ...
   ----------------------------------
   --  called after a :=
   --  pok when "obj.fun(n)"   "var[n+1].fun(x)"
   ---------------------------------
   function Is_Function_Call return Boolean is
      nesting: Natural :=0;
      Index: Natural := Current_Token;  --obj or var
   begin
      --   CANNOT UNDERSTAND WHY THIS CODE IS HERE !!!! BUGGED"""
      --    if Tokens(Index).Item.all /= "(" then
      --       return False;
      --    end if;
      if Tokens(Index+1).Item.all = "["  then  -- just after var
            ...
            if Tokens(Index).Item.all = "["  then
               ...
               while nesting /= 0 loop
                  if Tokens(Index).Item.all = "["  then
                    ...
                    Tokens(Index).Item.all = "}" or else
                    Tokens(Index).Item.all = "{" then
                     Give_Error ("6059 Errror: Found token " & Tokens(Index).Item.all &
                                   " while looking for a ""]""" );
                     raise Parsing_Error;
                  else
                     Index := Index+1;
      ...
      if Tokens(Index).Item.all  = "."  and then
        (Tokens(Index+1).Item.all /= "Length" and then
         ...
         Tokens(Index+1).Item.all /= "Head" and then
         Tokens(Index+1).Item.all /= "min" and then
         Tokens(Index+1).Item.all /= "Min" and then
         Tokens(Index+1).Item.all /= "max" and then
         Tokens(Index+1).Item.all /= "Max" and then -- APRIL 1
   ...
   begin
      ...
      if Current_item="exit" then
         ...
         if Current_Item = ":=" or else Current_Item = "=" then
            ...
            if Current_Item = "." then
               Give_Error("6414 Function calls cannot appear inside variable initializations or expressions");
               raise Parsing_Error;
            end if;
         else
         ...
         if This_Action.Assignment_Left = 0 then
            Give_Error ("6444 Error: Variable " & Prefix.all & " is not declared.");
            raise Parsing_Error;
         end if;
         while Current_Item = "[" loop
         ...
         if Is_Function_Call then
            ...
            The_Target:= Parse_SimpleExpr(TEnv.all,In_FunctCall => True);  --  "obj"
            ...
            if Current_Item  /= ";" and Current_Item  /= "}" then
               Give_Error("6511 function calls cannot appear inside other expressions");
               raise Parsing_Error;
            end if;
            This_Action.Signalled_Event.The_Args := My_Args;
         ...
         This_Action.Assignment_Left_Image := new String'(Prefix.all);
         if This_Action.Assignment_Left = 0 then
            Give_Error ("6561 Error: Variable " & Prefix.all & " is not declared.");
            raise Parsing_Error;
         end if;
         Skip_Token;   -- ++
         ...
         This_Action.Assignment_Left_Image := new String'(Prefix.all);
         if This_Action.Assignment_Left = 0 then
            Give_Error ("6601 Error: Variable " & Prefix.all & " is not declared.");
            raise Parsing_Error;
         end if;
         Skip_Token;    --
         This_Action.Assignment_Right := new umlExpr;
         ...
         Event_Name := Tokens(Current_Token).Item;
         if Event_Name.all = "tau" then
            Give_Error( "6638 ERROR:  'tau' is not allowed as event name,");
            raise Parsing_Error;
         end if;
         Skip_Token; -- "ev"
         --
      ...
      if Current_Item  = ";" then
        ...
        Current_Item = "}"  then
         ...
         Give_Error ("6707 Unxepected stuff after action: missing ""}"" , ""("", "";"" ?");
         raise Parsing_Error;
      end if;
      --  exception
      --     when others =>
   ...
   begin
      ...
      if Current_Item = "/" then
         if Next_Item ="-" then
            Give_Error("6883 Error: unexpected ""-"" at beginning of statements list");
            raise Parsing_Error;
         end if;
         Skip_Token;
         ...
         Give_Error("6891 Error: unexpected "";"" ");
         raise Parsing_Error;
      else
         Give_Error("6894 Error:  Transition syntax ::= ""{"" [trigger] [guard] [actions]""}"" ");
      ...
      if Is_Operation_Body and then not Return_or_Caller_Found then
         ...
         if This_Transition.Trigger.Return_Type = Undefined then
            ...
            Give_Error ("6920 Missing ""return"" action");
            raise Parsing_Error;
         end if;
      end if;
      --
   ...
   exception
      when others =>
         if PreviousPE = Parsing_errors then
            Give_Error("6930 Failure during parsing of transition structure");
         end if;
         if Parsing_Errors > 5 then
            raise Parsing_Error;
         else
   ...
   begin
      ...
      if Current_Item  = "(" then
         ...
         if This_Name /= null and then
           (This_Name.all = "initial" or else
            ...
            if (Current_Item /= "->") then
               Give_Error ("7003 no trigger / actions allowed for default initial transitions");
               raise Parsing_Error;
            end if;
         else
            DefInitial :=null;
      ...
      if Current_Item = ")->"  or else
        Current_Item = "->"  then
         ...
         Give_Error  ("7026 --> Transition target expected (found """& Current_Item & """, maybe missing "";""?) ");
         raise Parsing_Error;
      end if;
      --
      ...
      if Current_Item = "(" then
         ...
         if DefInitial /= null then
            if (Current_Item = "{") then
               Give_Error ("7048 no trigger / actions allowed for default initial transitions");
               raise Parsing_Error;
            elsif This_Transition.Target(1).parent /= DefInitial.parent then
               Give_Error ("7051 target state must be have the same parent state of the source state");
               raise Parsing_Error;
            end if;
      ...
      begin
         ...
         if Tmp_Owner = null then
            Give_Error ("7140 Error in State Name???");
            raise Parsing_Error;
         else
            --  THIS IS A BUG
            ...
            if Tmp_Owner.Kind = Parallel then
               Give_Error ("7157 Transitions inside a parallel state cannot connect different regions.");
               raise Parsing_Error;
            end if;
         end if;
      end;
   ...
   begin
      if Current_Item = "Transitions" or else
        ...
        Current_Item = "Behavior" then
         ...
         Give_Error("7237 I was Expecting 'Transitions' (or 'Behavior'), and found '" &
                      Current_Item &"' instead.");
         raise Parsing_Error;
      end if;
      if Current_item = ":" then
         Skip_Token;
