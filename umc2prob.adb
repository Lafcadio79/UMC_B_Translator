...
--   OPNAMESfromTLABELS: Boolean := True;
   ----------------------------
  ...
  begin
     if This_Simple.Special_Token = null then
           ...
           elsif This_Simple.Image.all = "true" then -- true is not a Special Token !?!
              return "TRUE";
           elsif This_Simple.Image.all = "false" then --  false is not Special Token ?!?
              return "FALSE";
           else
              return  This_Simple.Image.all;   -- Token identifiers !!!
           end if;
     ...
     elsif This_Simple.Special_Token.all = "true" then -- should not happen true is Special Token
              return "TRUE";
     elsif This_Simple.Special_Token.all = "false" then -- should not happen false is Special Token
            return "FALSE";
     end if;
    --  
    if This_Simple.Local_Variable /= 0 then
        ...
        return To_Upper(This_Simple.Special_Token.all);
    else
      declare
  ...
  begin
            ...
            -- SENDING of SIGNALS
               ...
               if GetTarget(Actions(A)) /= "OUT" then
                 ...
                 if Actions(A).Signalled_Event.The_Event.Params.all'Length >0  then
                    Put_line(";");
                    for Z in Actions(A).Signalled_Event.The_Args'Range loop
                       Put(Space(1..I) &
                       GetTarget(Actions(A)) & "_databuff := " & 
                       GetTarget(Actions(A)) & "_databuff <- " & 
                       UmlExpr_img(obj, Actions(A).Signalled_Event.The_Args(Z)) );
                       if Z < Actions(A).Signalled_Event.The_Args'Last then
                          Put_line(";");
                       end if;
                    end loop;
                 end if;
   ...
   function Get_SignalName_and_Args (sd: Natural; K: Natural) return String is
     Actions: Actions_Table_Ref := All_Charts(SD).ChartTransitions(K).Actions;
     Str: String_Ref;
   begin
      ...
      for A in Actions'Range loop
         if Actions(A).Kind = Signal then
            Str := new STring'(Actions(A).Signalled_Event.The_Event.Name.all);
            if Actions(A).Signalled_Event.The_Event.Params.all'Length >0  then
               Str := new String'("(");
               for I in Actions(A).Signalled_Event.The_Event.Params.all'range loop
                  Str := new String'(Actions(A).Signalled_Event.The_Event.Params(I).Name.all);
                  if I < Actions(A).Signalled_Event.The_Event.Params.all'Last then
                     Str := new String'(",");
                  end if;
               end loop;
               Str := new String'(")");
   ...
   end Get_SignalName_and_Args;
   
   function Get_OPNAME (sd: Natural; K: Natural) return String is
   begin
--      if OPNAMESfromTLABELS then
         --operation Names generated from transition labels
         if All_Charts(SD).ChartTransitions(K).label /= null then
		    -- print transition labels
            return All_Charts(SD).ChartTransitions(K).label.all;
         else
            return All_Charts(SD).Name.all & "_nolabel" & Trim(K'Img, Left) & " =";
         end if;  -- label /= null
   end get_OPNAME;
   ...
   begin
      ...
      for K in This_Chart.ChartTransitions'Range loop
         ...
         if This_Chart.ChartTransitions(K).trigger /= null and then
            ...
            HasTrigger := True;
            ArgsCount := This_Chart.ChartTransitions(K).Trigger.Params.all'Length;
         end if;
         New_Line;
         ...
         if ArgsCount >0 then 
            Put("(");
            for I in This_Chart.ChartTransitions(K).Trigger.Params.all'range loop
               Put(This_Chart.ChartTransitions(K).Trigger.Params(I).name.all);
               if I < This_Chart.ChartTransitions(K).Trigger.Params.all'last then
                  Put(",");
               end if;
            end loop;
            Put(")");
         ...
         if HasTrigger then
            ...
            if ArgsCount >0 then
              for I in This_Chart.ChartTransitions(K).Trigger.Params'Range loop
                 -- arganame1 =  first(obj_databuff);
                 -- arganame2 =  first(tail((obj_databuff));
                 Put("  " & This_Chart.ChartTransitions(K).Trigger.Params(I).name.all &
                      " = first(");
                 for II in 2..I loop
                    Put("tail(");
                 end loop;
                 Put(This_Chart.Name.all & "_databuff");
                 for III in 1..I loop
                    Put(")");
                 end loop;
                 Put_Line (" &");
...
begin
   if Ada.Command_Line.Argument_Count = 0  then
      Put_Line (" Usage:   umc2B   UMCFilename [options] [B_machine_name] [result.mch]");
      Put_Line ("          option: -v  (version and help)");
      return;
   end if;

   for X in 1..Ada.Command_Line.Argument_Count loop
      if  Ada.Command_Line.Argument(X) = "-v" then
         Put_Line (" umc2B version 4b (feb 29 2021)");
         Put_Line (" usage:   umc2B   UMCFilename [options] [B_machine_name] [result.mch]");
         Put_Line ("          option: -v  (version and help)");
         return;
      elsif UMCfilename = null then
         UMCfilename  := new String'(Ada.Command_Line.Argument(X));
         B_Filename := new String'(UMCfilename.all(1 .. UMCfilename'Length - 4) & ".mch");
      elsif index(Ada.Command_Line.Argument(X), ".") >0 then
         B_Filename := new String'(Ada.Command_Line.Argument(X));
      else 
        Machine_name := new String'(Ada.Command_Line.Argument(X));
      end if;
   end loop;
   --
   if UMCfilename = null then
      Put_Line (" Usage:   umc2b   UMCFilename [options] [LNTfilename]");
      Put_Line ("          option: -v  (version and help)");
      return;
   end if;
   ...
   for I in All_Charts'Range loop
      if Is_Active_Chart(I) then
         Put_line("////////////////////////////////////// ");
         Put_line("// State Machine " & All_Charts(I).Name.all );
         Put_line("////////////////////////////////////// ");
         print_transitions(I);
